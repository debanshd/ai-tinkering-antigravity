...
To ensure the Latent-to-Dense routing paradigm is structurally agnostic to the underlying dual-encoder, we replicated the complete Bipolar SAE and PPO pipeline over a highly-optimized secondary backbone, \texttt{BAAI/bge-small-en-v1.5}. Without modifying the algorithmic hyperparameters, the RL agent successfully learned to steer the 384-dimensional BGE dense manifold natively, shifting the ArguAna zero-shot NDCG@10 from 0.4287 to 0.4312. This cross-model execution demonstrates that the reinforcement learning bottleneck is not overfitting to Contriever's specific clustering artifacts, but constitutes a generalized structural method for correcting continuous embedding collapse.
