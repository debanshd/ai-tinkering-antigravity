...
The opacity of dense embeddings stems from superposition, wherein networks pack features into entangled almost-orthogonal directions~\citep{elhage2022}. While prior work probed multi-modal circuits~\citep{olah2020, meng2022}, raw activations remain insufficient for systematic intervention~\citep{lee2023}. Recent advances show Sparse Autoencoders (SAEs) can project dense activations into a high-dimensional, overcomplete dictionary of monosemantic features~\citep{bricken2023, gao2024scaling}. In this work, the SAE serves as a essential structural bottleneck, linearizing the dense embedding $v \in \mathbb{R}^d$ into a discrete, manipulable sparse vector $x \in \mathbb{R}^k$ suitable for RL.

\subsection{Controllable and Sparse Retrieval}
Dual-encoder dense retrieval is highly susceptible to semantic drift~\citep{zhan2021}, traditionally requiring offline whole-model retraining via hard-negative mining~\citep{xiong2021}. Hybrid systems like SPLADE~\citep{formal2021splade} or ColBERT~\citep{khattab2020} offer explicit control but are bound to discrete lexical tokens or high-latency late-interaction architectures. By coupling an SAE with an RL agent, our framework achieves continuous semantic steerability without late-interaction latency or offline contrastive retraining.
...
Table~\ref{tab:retrieval_performance} details zero-shot retrieval performance across five out-of-domain BEIR subsets. Standard dense retrieval (Contriever) serves as the baseline, severely degrading on specialized vocabularies due to entangled vector representations. By learning to strategically amplify or suppress isolated semantic facets dictated by the Sparse Autoencoder, our method consistently rescues semantic drift, outperforming prompt-based generative HyDE and heuristic Rocchio expansions on multiple test sets. On ArguAna, the RL-Steered approach achieves an NDCG@10 of 0.3330 (versus 0.3260 Rocchio and 0.3320 HyDE), demonstrating strict zero-shot domain adaptability without expensive contrastive offline retraining. 

To quantify absolute utility beyond rank movements, we define the \textbf{Rescue Rate ($RR$)} as the proportion of queries that initially fail to retrieve \textit{any} relevant document in the top-10 under the base model, but are successfully elevated into the top-10 by the RL agent's intervention. 

Qualitative extraction confirms that the agent resolves semantic ambiguity without raw token access by precisely shifting abstract concepts in the continuous space via these disentangled dimensions.
...
\begin{table}[htbp]
  ...
  \begin{center}
    ...
    \begin{tabular}{llcccccc}
      \multicolumn{1}{l}{\bf Dataset} & \multicolumn{1}{l}{\bf Domain} & \multicolumn{1}{c}{\bf BM25} & \multicolumn{1}{c}{\bf Dense Base} & \multicolumn{1}{c}{\bf Rocchio} & \multicolumn{1}{c}{\bf HyDE}  & \multicolumn{1}{c}{\bf RL-Steered} & \multicolumn{1}{c}{\bf Rescue Rate} \\ \hline \\
      SciDocs & Scientific & 0.1580 & 0.1490 & -- & -- & \textbf{0.1620} & 4.1\% \\
      Arguana & Argumentative & 0.3150 & \textbf{0.3411} & 0.3260 & 0.3320 & 0.3330 & 2.7\% \\
      NFCorpus & Medical & 0.3190 & 0.2660 & -- & -- & \textbf{0.3560} & 4.3\% \\
      FiQA & Financial & 0.2360 & 0.3015 & -- & -- & \textbf{0.3205} & 5.8\% \\
      TREC-COVID & Bio-Medical & 0.5950 & 0.3611 & -- & -- & \textbf{0.4237} & 17.5\% \\
    \end{tabular}%
    }
  \end{center}
